...
import Teleop from '@site/static/gif/teleop.gif';
...
<img src={Teleop} alt="Teleoperation demo gif" style={{ width: '100%', maxWidth: 700 }} />
...
4. The `teleop_twist_keyboard` node will publish velocity commands according to
   ...
   ```bash
   ros2 launch message_transforms message_transforms.launch.yaml parameters_file:=<path/to/transforms.yaml>
   ```
   ...
   ```bash
   ros2 launch message_transforms message_transforms.launch.yaml parameters_file:=./blue_demos/teleoperation/config/transforms.yaml
   ```
