...
1. Launch the demo BlueROV2 in simulation by running the following command:
   ...
   `adaptive_integral_terminal_sliding_mode_controller` (AITSMC) controller.
   The control framework uses the `thruster_hardware/ThrusterHardware` system
   interface to enable thruster-level control with ArduSub and the
   `topic_sensors/OdomSensor` sensor interface convert ROS 2 messages into
   a ros2_control compatible format.

   :::tip

   On occasion, MAVROS or the ros2_control controller manager will fail to load.
   If this happens, stop the launch and try again.

   :::


2. Wait for Gazebo, ArduSub, ros2_control and MAVROS to fully load. Typically,
   the `adaptive_integral_terminal_sliding_mode_controller` will be the last
   component to load.

3. Open a new terminal and activate the control system:

   ```bash
   ros2 service call /controller_coordinator/activate std_srvs/srv/SetBool "{data: true}"
   ```
